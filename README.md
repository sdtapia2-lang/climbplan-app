...
## Onboarding de escalador nuevo (Fase 8)

Reemplaza la pantalla "pedile a un Admin que te asigne un rol" por dos caminos de autoservicio, apenas alguien se registra (por email o por Google):

- **Entrenamiento libre**: la cuenta se vuelve su propio "entrenador" — internamente se inserta un `coach_athletes` con `coach_id` = su propio id. No hace falta ninguna policy nueva de escritura sobre mesociclos: `is_my_assigned_athlete()` ya le da los mismos permisos que a cualquier Entrenador sobre su propio atleta. Antes de poder planificar, la app la obliga a completar una **evaluación física** (solo a estas cuentas — se marcan con `profiles.onboarded_via_free`, el resto de los flujos no cambia).
- **Buscar entrenador**: elige un Entrenador del directorio (Fase 6) y le manda una "solicitud de entrevista" (`coach_requests`, con un mensaje opcional). El Entrenador (o un Admin) la ve en **Solicitudes** (`/solicitudes`) y la acepta o rechaza. Aceptar corre `accept_coach_request()` — una función `SECURITY DEFINER` porque hace falta escribir en `profiles` (solo el Admin puede vía RLS normal), crear el `athletes` y el `coach_athletes`. El escalador queda **sin restringir**: puede modificar su propia planificación, a diferencia del escalador libre de la Fase 4 (que lo crea directamente el Entrenador).
- La asignación manual desde el panel de Admin (Fase 1) sigue funcionando exactamente igual — esto es un camino adicional, no un reemplazo.
...
4. Ejecutá [`supabase/seed_exercises_lattice.sql`](supabase/seed_exercises_lattice.sql) para cargar el catálogo real de Lattice Training (194 ejercicios, extraídos y verificados directamente de capturas de la app — 6 categorías: Aerobic Base, Conditioning, Power Endurance, Strength and Power, Flexibility, Fingerboard). Este script borra y reemplaza cualquier catálogo anterior; no hace falta correr `seed_exercises.sql` (el catálogo chico exportado de Base44, ya obsoleto).
...
11. Ejecutá [`supabase/phase8_free_training_and_requests.sql`](supabase/phase8_free_training_and_requests.sql) para el onboarding de escalador nuevo (entrenamiento libre + solicitudes de entrevista).
12. Andá a **Project Settings → API** y copiá:
   - **Project URL**
   - **anon public key**
...
supabase/
  ...
  seed_exercises_lattice.sql # catalogo real de Lattice Training (194 ejercicios verificados, 6 categorias)
